// REQUIRES: nvptx
// RUN: lego-opt %s --lego-to-nvvm='format=assembly' | FileCheck %s

// Test: LEGO layout ops inside gpu.launch lower through NVVM/CUDA pipeline.
// No NVIDIA GPU hardware required — this is a compiler-only test.
...
// Verify the module lowered to NVVM — the gpu.binary op should contain
// the compiled kernel object (PTX assembly).
// CHECK: gpu.binary
