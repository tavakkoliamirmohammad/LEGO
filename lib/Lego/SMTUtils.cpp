...
/// Compute warp addresses for a GenP layout by evaluating its body region.
static LogicalResult computeWarpAddressesGenP(
    Operation *apply, GenPOp genP, ValueRange indices,
    SMTSolverContext &smtCtx, AsmState &state, unsigned &nextId,
    int warpSize, Value baseThread,
...
/// Compute warp addresses for non-GenP layouts by materializing arith
/// ops via applyLayout() (the same lowering used in lego-lower), then
/// encoding the resulting arith computation graph into SMT.
static LogicalResult computeWarpAddressesNonGenP(
    Operation *apply, Value layout, ValueRange indices,
    SMTSolverContext &smtCtx, AsmState &state, unsigned &nextId,
    int warpSize, Value baseThread,
    std::optional<BlockArgument> threadArg,
    SmallVectorImpl<Value> &addresses) {
  OpBuilder &smtBuilder = *smtCtx.b;
  Location loc = apply->getLoc();

  // Materialize arith ops at the ApplyOp's location using the same
  // layout lowering that lego-lower uses.
  OpBuilder arithBuilder(apply->getContext());
  arithBuilder.setInsertionPoint(apply);
  Value flatArith = lego::applyLayout(arithBuilder, loc, layout, indices);
  if (!flatArith)
    return failure();

  // For each thread, substitute the thread ID into the arith computation
  // graph and encode the result into SMT.
  for (int t = 0; t < warpSize; ++t) {
    Value tConst = smt::IntConstantOp::create(smtBuilder, loc, smtBuilder.getI64IntegerAttr(t));
    Value threadId = smt::IntAddOp::create(smtBuilder, loc, ValueRange{baseThread, tConst});

    DenseMap<Value, Value> threadValMap;
    if (threadArg)
      threadValMap[*threadArg] = threadId;

    SMTBuilder threadSMT(smtBuilder, state, nextId);
    threadSMT.valMap = threadValMap;

    // Encode the materialized arith flat index into SMT.
    // getOrCreate walks the arith def-use chain and converts each
    // arith op to its SMT equivalent.
    Value smtAddr = threadSMT.getOrCreate(flatArith);
...
LogicalResult computeWarpAddresses(
  ...
  // Dispatch based on layout type.
  auto genP = dyn_cast_or_null<GenPOp>(layout.getDefiningOp());
  if (genP && !genP.getBody().empty()) {
    return computeWarpAddressesGenP(apply, genP, indices, smtCtx, state,
                                    nextId, warpSize, baseThread, threadArg,
                                    addresses);
  }

  // Non-GenP layouts: compute flat addresses via direct SMT encoding.
  Operation *defOp = layout.getDefiningOp();
  if (!defOp)
    return failure();

  if (isa<RegPOp, OrderByOp, GroupByOp, TileByOp>(defOp)) {
    return computeWarpAddressesNonGenP(apply, layout, indices, smtCtx, state,
                                       nextId, warpSize, baseThread, threadArg,
                                       addresses);
  }

  // Unknown layout type.
  apply->emitWarning("SMT verification skipped: unsupported layout type '")
      << defOp->getName() << "'";
  return failure();
}
