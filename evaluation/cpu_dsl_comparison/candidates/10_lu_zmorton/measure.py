"""Measure 10_lu_zmorton: LU-style gather with Z-Morton layout.

CASTLE candidate 02 (polybench-lu-zmorton). Prior: AMD WIN, Intel WIN.
"""
import json
import math
import sys
...
def main():
    ...
    A = rng.standard_normal(N_BENCH).astype(np.float32)
    B = rng.standard_normal(N_BENCH).astype(np.float32)
    C_ref = np.zeros(N_BENCH, dtype=np.float32)
    ...
    print(json.dumps({"name": "10_lu_zmorton", "N": N_BENCH,
                      "layout_class": "Z-Morton", "prior_verdict": "WIN",
                      ...
                      "speedup_isolated_jit": sp_iso, "speedup_vs_numpy": sr(t_numpy, t_vec),
