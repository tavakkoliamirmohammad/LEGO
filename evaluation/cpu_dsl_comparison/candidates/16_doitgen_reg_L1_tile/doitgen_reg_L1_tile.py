"""16_doitgen_reg_L1_tile: Doitgen tensor update proxy with register + L1 tiling.

CASTLE candidate 8. Layout class: Reg+L1 tile.
...
@cpu_kernel(grid=(N,))
def doitgen_reg_L1_tile(A: Buffer[N], B: Buffer[N], C: Buffer[N]):
    for i in range(N):
        C[i] = A[i] * B[i] + C[i]
...
if __name__ == "__main__":
    A, B, C = _make_args()
    rec = doitgen_reg_L1_tile.measure(A, B, C)
    rec["verified"] = doitgen_reg_L1_tile.verify(A, B, np.zeros_like(C))
