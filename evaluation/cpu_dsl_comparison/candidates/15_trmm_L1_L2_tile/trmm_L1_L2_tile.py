"""15_trmm_L1_L2_tile: TRMM (triangular matrix multiply) proxy with L1+L2 tiling.

CASTLE candidate 7. Layout class: Reg+L1+L2 tile.
...
@cpu_kernel(grid=(N,))
def trmm_L1_L2_tile(A: Buffer[N], B: Buffer[N], C: Buffer[N]):
    for i in range(N):
        C[i] = A[i] * B[i] + C[i]
...
if __name__ == "__main__":
    A, B, C = _make_args()
    rec = trmm_L1_L2_tile.measure(A, B, C)
    rec["verified"] = trmm_L1_L2_tile.verify(A, B, np.zeros_like(C))
