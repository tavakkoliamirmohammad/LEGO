"""30_lulesh_aosoA: LULESH element-centered with AoSoA layout.
...
@cpu_kernel
def lulesh_aosoA(A: Buffer[N_BUF], B: Buffer[N]):
    for i in range(N):
        B[i] = A[i * 4] * 2.0
...
if __name__ == "__main__":
    A, B = _make_args()
    rec = lulesh_aosoA.measure(A, B)
    rec["verified"] = lulesh_aosoA.verify(A, np.zeros_like(B))
