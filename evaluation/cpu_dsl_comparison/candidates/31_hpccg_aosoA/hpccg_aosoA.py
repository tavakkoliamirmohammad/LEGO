"""31_hpccg_aosoA: HPCCG CG solver with AoSoA layout.

CASTLE candidate 23. Layout class: AoSoA.
...
@benchmark(
    ...
    meta={"N": N, "layout_class": "AoSoA", "prior_verdict": "LOSS"},
)
@cpu_kernel
def hpccg_aosoA(A: Buffer[N_BUF], B: Buffer[N]):
    for i in range(N):
        B[i] = A[i * 4] * 2.0
...
if __name__ == "__main__":
    A, B = _make_args()
    rec = hpccg_aosoA.measure(A, B)
    rec["verified"] = hpccg_aosoA.verify(A, np.zeros_like(B))
