"""Measure 36_gemm_nonpow2_morton.
...
def main():
    ...
    t_scalar = float("nan")
    try:
        t_scalar = _bench.bench_self_timed(A_np, B_np, C_np, n_iters=200, n_warmup=50, target="scalar")
    except Exception:
        pass
    ...
    notes = ""
    try:
        t_vec = _bench.bench_self_timed(A_np, B_np, C_np, n_iters=200, n_warmup=50, target="x86")
    ...
    print(json.dumps({
        "name": "36_gemm_nonpow2_morton",
        "N": N_BENCH,
        "layout_class": "Morton+non-pow2",
        "prior_verdict": "WIN",
