import sympy as sp
import jinja2
from lego_python import *


def input_layout(n_rows, n_cols, BLOCK_SIZE):
    M = OrderBy(Row(n_rows, n_cols)).TileBy([n_rows, n_cols])
    return M['row_idx', get_arange(0, BLOCK_SIZE)]


def output_layout(n_rows, n_cols, BLOCK_SIZE):
    M = OrderBy(Row(n_rows, n_cols)).TileBy([n_rows, n_cols])
    return M['row_idx', get_arange(0, BLOCK_SIZE)]


n_rows, n_cols, BLOCK_SIZE = sp.symbols(
    'n_rows n_cols BLOCK_SIZE', integer=True, positive=True)
input_offset_code = input_layout(n_rows, n_cols, BLOCK_SIZE)
output_offset_code = output_layout(n_rows, n_cols, BLOCK_SIZE)
...
params = {
    'input_offest': input_offset_code,
    'output_offset': output_offset_code,
}
printer = LEGOPythonCodePrinter(allow_unknown_functions=True)
render_params = {key: printer.doprint(
    sp.simplify(value)) for key, value in params.items()}
