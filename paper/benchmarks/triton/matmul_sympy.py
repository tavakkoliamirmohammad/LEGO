...
def parse_arguments():
    ...
    parser.add_argument('--TA', action='store_true', help='Set transpose_a to True')
    parser.add_argument('--TB', action='store_true', help='Set transpose_b to True')
    parser.add_argument('--NTA', action='store_true', help='Set transpose_a to False')
    parser.add_argument('--NTB', action='store_true', help='Set transpose_b to False')
    return parser.parse_args()

args = parse_arguments()
transpose_a = args.TA if args.TA else not args.NTA
transpose_b = args.TB if args.TB else not args.NTB
...
def get_autotune_config():
    return get_cuda_autotune_config() + get_hip_autotune_config()

# The abstract definition of the matmul kernel
def matmul_kernel(
        a_ptr, b_ptr, c_ptr,
        M, N, K,
    ...
    L_A = OrderBy(Col(M, K) if transpose_a else Row(M, K)).TileBy([M/BM, K/BK], [BM, BK])
    L_B = OrderBy(Col(K, N) if transpose_b else Row(K, N)).TileBy([K/BK, N/BN], [BK, BN])
...
kernel_source = lego.get_kernel_source(matmul_kernel)
ta_str = ".T" if transpose_a else ""
tb_str = ".T" if transpose_b else ""
...
torch.manual_seed(0)
a = torch.randn((512, 512), device='cuda', dtype=torch.float16){ta_str}
b = torch.randn((512, 512), device='cuda', dtype=torch.float16){tb_str}
...
def benchmark(M, N, K, provider, fp8_inputs):
    a = torch.randn((M, K), device='cuda', dtype=torch.float16){ta_str}
    b = torch.randn((K, N), device='cuda', dtype=torch.float16){tb_str}
