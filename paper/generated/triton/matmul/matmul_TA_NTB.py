...
def matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BK)):
        a_ptrs = a_ptr + BM*pid_m + M*(BK*k + ((tl.arange(0, BK))[None, :])) + ((tl.arange(0, BM))[:, None])
        b_ptrs = b_ptr + BN*pid_n + N*(BK*k + ((tl.arange(0, BK))[:, None])) + ((tl.arange(0, BN))[None, :])
        a = tl.load(a_ptrs)
        b = tl.load(b_ptrs)
...
torch.manual_seed(0)
a = torch.randn((512, 512), device='cuda', dtype=torch.float16).T
...
def benchmark(M, N, K, provider, fp8_inputs):
    a = torch.randn((M, K), device='cuda', dtype=torch.float16).T
