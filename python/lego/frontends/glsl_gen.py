"""GLSL source code generation adapter for the LEGO rewriter."""

import ast

from lego.glsl_printer import LEGOGLSLCodePrinter
from lego.frontends._adapter import DSLAdapter
from lego.rewriter import rewrite


class GLSLAdapter(DSLAdapter):
    """Adapter that emits GLSL source code from LEGO layout expressions."""

    def unwrap(self, fn):
        return fn, fn, []

    def find_runtime_vars(self, func_def):
        return set()

    def get_code_printer(self):
        return LEGOGLSLCodePrinter()

    def compile_and_wrap(self, new_source, tree, original_fn, wrappers,
                         return_source=False):
        return new_source


def generate(fn, **kwargs):
    """Generate GLSL source code from a function with LEGO layout expressions.

    Useful for GPU shader code generation.

    Usage::

        glsl_code = lego.glsl_gen.generate(index_kernel)
    """
    return rewrite(fn, GLSLAdapter(), return_source=True, **kwargs)
